...
X.loader.prototype.checkFileFormat = function(container) {
  ...
  var extension = filepath.split('.').pop().toUpperCase().split('?')[0];
  ...
  if (!(extension in X.loader.extensions)) {
    ...
    extension = 'STL';
    window.console.log('Unknow extension. Set default .stl');
...
X.loader.extensions = {
  ...
  'GZ': [X.parserGZ, null], // right now stl.gz is the only
  // format ending .gz
