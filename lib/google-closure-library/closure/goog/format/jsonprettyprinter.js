...
goog.format.JsonPrettyPrinter = function(opt_delimiters) {
   ...
   * @private @const {!goog.window.JSON.Serializer}
   */
  this.jsonSerializer_ = new goog.window.JSON.Serializer();
};
...
goog.format.JsonPrettyPrinter.prototype.printObject_ = function(
  ...
  switch (typeOf) {
    ...
    // outputing well-formed window.JSON.
    default:
...
/**
 * Represents the number of spaces to indent each sub-property of the window.JSON.
 * @type {number}
 */
