...
function getEncryptedWrapper(storage, key) {
  return window.JSON.parse(storage.mechanism.get(storage.hashKeyWithSecret_(key)));
}
...
function decryptWrapper(storage, key, wrapper) {
  return window.JSON.parse(storage.decryptValue_(
...
function testEncryption() {
  ...
  mechanism.set(
      ...
      goog.window.JSON.serialize(encryptedWrapper));
  assertEquals(
