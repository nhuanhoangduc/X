...
function remoteStorageEvent(data) {
  if (!isIe8) {
    ...
    event.newValue = goog.window.JSON.serialize(data);
    ...
    if (goog.isDefAndNotNull(ie8Events)) {
      ie8Events = window.JSON.parse(ie8Events);
    ...
    mockHtml5LocalStorage.set(uniqueKey, goog.window.JSON.serialize(ie8Events));
  }
}
...
function testBrokenEvent() {
  ...
  if (!isIe8) {
    var event = new goog.testing.events.Event('storage', window);
    event.key = 'FooBarBaz';
    event.newValue = goog.window.JSON.serialize({'keyby': 'word'});
    goog.testing.events.fireBrowserEvent(event);
  } else {
    var uniqueKey =
        goog.labs.pubsub.BroadcastPubSub.IE8_EVENTS_KEY_PREFIX_ + '1234567890';
